...
\begin{solution}
The objective is to have more cards in our pile than the house, so we want some way to ``burn'' black cards. However, the only way that black cards (and symmetrically, red cards) can be used up without awarding points to the house is if they are discarded. Note, however, that this requires a red card burn too. Thus, there is \textit{no} way to build an advantage. In fact, every game will always end in a tie (though the amount of discarded and held cards may differ game to game). The game is unwinnable, so no amount of prize money makes it worth playing. 
\end{solution}
